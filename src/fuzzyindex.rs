    ...
    pub fn matches<M: Metric<Point>>(
        ...
        for (word_idx, _) in words_freqs.iter().take(params.max_comparisons) {
            ...
            if true {
                ...
                if jaccard > 0.01 {
                    ret.insert(word_b_info.label.clone(), jaccard);
                }
                if jaccard > return_if_gt {
                    //ret.push((word_b_info.label.clone(), jaccard));
                    break;
                }
                if jaccard.gt(&max_so_far) {
                    //ret.push((word_b_info.label.clone(), jaccard));
        ...
        let mut retvec: Vec<(Id, f64)> = ret.into_iter().collect();
        retvec.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());
        retvec
    }
}
