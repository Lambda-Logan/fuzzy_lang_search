...
impl<P: FuzzyPoint> Metric<P> for Total {
    fn dist(&self, p1: &P, p2: &P) -> f64 {
        let sim = self.sim(p1, p2);
        if sim > 0.0 {
            1.0 / sim
...
impl<P: FuzzyPoint> Metric<P> for Cosine {
    fn dist(&self, p1: &P, p2: &P) -> f64 {
        let sim = self.sim(p1, p2);
        if sim > 0.0 {
            1.0 / sim
    ...
    fn sim(&self, p1: &P, p2: &P) -> f64 {
        ...
        let left_norm = 1.0 / (left as f64).sqrt();
        let right_norm = 1.0 / (right as f64).sqrt();
        ((dot as f64) * left_norm * right_norm) / (total as f64)
    }
}
